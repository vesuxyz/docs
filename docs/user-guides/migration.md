...
Affected positions show a **Withdraw** button.
![Withdraw button](./images/withdraw-button.png)

1. Withdraw your `USDC.e`  
2. Swap `USDC.e` to `USDC` on [Ekubo](https://ekubo.org/starknet/swap) or [Avnu](https://app.avnu.fi) when liquidity is available. Otherwise, bridge `USDC.e` via the official [Starkgate bridge](https://starkgate.starknet.io) and convert it on another chain.
