---
title: Vesu USDC Alterscope xSTRK Risk Report
sidebar_label: vUSDC-Alterscope xSTRK
sidebar_position: 2
slug: /curators/risk-reports/1129317009595740662798266590704690378283717359139782594834794341553978002262/usdc
---
...
The *Vesu USDC Alterscope xSTRK* market represents `USDC` deposits in Alterscope's *Alterscope xSTRK* pool.

The pool enables `xSTRK` as collateral asset to borrow a variety of loan assets including `ETH`, `WBTC`, `STRK`, `USDC`, `USDT`. It uses Vesu's official factory extension with Pragma price feeds.
...
`USDC` deposits can be borrowed with `xSTRK` as collateral.
...
The pool enables borrowing of `USDC` only with `xSTRK` collateral, which itself achieves a "Medium" rating. Collateral *rehypothecation* is not enabled in the pool. 
