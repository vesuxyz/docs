---
title: Vesu xSTRK Alterscope xSTRK Risk Report
sidebar_label: vxSTRK-Alterscope xSTRK
sidebar_position: 2
slug: /curators/risk-reports/1129317009595740662798266590704690378283717359139782594834794341553978002262/xstrk
---
...
The *Vesu xSTRK Alterscope xSTRK* market represents `xSTRK` deposits in Alterscope's "Alterscope xSTRK" pool.
...
At the time of writing this report, almost 35M `STRK` is staked with Endur making it the largest `STRK` staker and dominant `STRK` LST protocol.
...
The pool uses Pragma's `xSTRK/USD` price feed.

The Pragma oracle is a native Starknet oracle system that secured up to USD 200m across all major projects on Starknet.
...
Since `xSTRK` is only traded on the Ekubo DEX on Starknet, all providers ultimately rely on the same price source.
...
`xSTRK` deposits in this pool are isolated from borrowing removing market risk for lenders.
